...
def dashboard():
    ...
    if "MOTO" not in acusados_disponibles: acusados_disponibles = [acusado for acusado in acusados_disponibles if acusado != "MOTO"]
    ...
    if edad_min_filtro is not None and edad_max_filtro is not None: filtro_edad = list(range(edad_min_filtro, edad_max_filtro + 1))
    elif edad_min_filtro is not None: filtro_edad = list(range(edad_min_filtro, max(edades_unicas) + 1))
    elif edad_max_filtro is not None: filtro_edad = list(range(min(edades_unicas), edad_max_filtro + 1))
    else: filtro_edad = None
    if ano_min_filtro is not None and ano_max_filtro is not None: filtro_ano = list(range(ano_min_filtro, ano_max_filtro + 1))
    elif ano_min_filtro is not None: filtro_ano = list(range(ano_min_filtro, max(anos_unicos) + 1))
    elif ano_max_filtro is not None: filtro_ano = list(range(min(anos_unicos), ano_max_filtro + 1))
    else: filtro_ano = None
    if mes_min_filtro is not None and mes_max_filtro is not None: filtro_mes = list(range(mes_min_filtro, mes_max_filtro + 1))
    elif mes_min_filtro is not None: filtro_mes = list(range(mes_min_filtro, max(meses_unicos) + 1))
    elif mes_max_filtro is not None: filtro_mes = list(range(min(meses_unicos), mes_max_filtro + 1))
    else: filtro_mes = None
    ...
    with st.spinner('Cargando datos...'):
        ...
        boxplot = ax.boxplot(df_filtrado["HORA"].dt.hour,vert=False,labels=[""],medianprops=dict(color="Black", linewidth=3),)
        ...
        quartiles = df_filtrado["HORA"].dt.hour.quantile([0.25, 0.5, 0.75])
        for i, quartile in enumerate(quartiles): ax.text(quartile, 1.08, f"{int(quartile)} Hrs", fontsize=7, color='black', ha='center', va='bottom')
        st.pyplot(fig)
        ...
        fig, ax = plt.subplots(figsize=(8, 3))
        boxplot = ax.boxplot(df_filtrado["EDAD"],vert=False,labels=[""],medianprops=dict(color="black", linewidth=3),)
        ax.set_xlabel("Edad de la víctima")
        ax.set_title("Edad del fallecido en accidentes con resultados fatales")
        ax.set_facecolor("lemonchiffon")
        ax.grid(True, color="lightgray", linewidth=0.5, zorder=0)
        quartiles = df_filtrado["EDAD"].quantile([0.25, 0.5, 0.75])
        for i, quartile in enumerate(quartiles): ax.text(quartile, 1.08, f"{int(quartile)} años", fontsize=7, color='black', ha='center', va='bottom')
        st.pyplot(fig)
        ...
        for i in range(50, max_value, 50): ax.axhline(y=i, color='lightgray', linewidth=0.5, zorder=0)
        for i, valor in enumerate(top_5_acusados): ax.text(i, valor, f"{valor:.2f}%", ha="center", va="bottom")
        plt.tight_layout()
        st.pyplot(fig)
        ...
        top_5_victimas = df_filtrado["VICTIMA"].value_counts(normalize=True).nlargest(5) * 100  # Calculamos los porcentajes
        ...
        frecuencia_acusado = df_filtrado["ROL"].value_counts(normalize=True) * 100  # Calculamos los porcentajes
        fig, ax = plt.subplots(figsize=(10, 5))
        ...
        ax.set_facecolor("lemonchiffon")
        for i in range(50, math.ceil(max(frecuencia_acusado)), 50): ax.axhline(y=i, color='lightgray', linewidth=0.5, zorder=0)
        for i, valor in enumerate(frecuencia_acusado): ax.text(i, valor, f"{valor:.2f}%", ha="center", va="bottom")
        st.pyplot(fig)
        ...
        frecuencia_victima = df_filtrado["SEXO"].value_counts(normalize=True) * 100  # Calculamos los porcentajes
        plt.figure(figsize=(10, 5))
        ...
        for i in range(100, max_value, 100): plt.axhline(y=i, color='lightgray', linewidth=0.5, zorder=0)
        for i, valor in enumerate(frecuencia_victima): plt.text(i, valor, f"{valor:.2f}%", ha="center", va="bottom")
        st.pyplot(plt)
        ...
        sns.kdeplot(data=df_filtrado, x='EDAD', hue='SEXO', fill=True, common_norm=False, palette='gray_r')
        plt.xlabel('Edad de la víctima')
        plt.ylabel('Densidad')
        plt.title('Distribución de edades de las víctimas por sexo')
        plt.ylim(0, 0.03)  
        plt.gca().set_facecolor("lemonchiffon")
        for i in np.arange(0, 0.03, 0.005): plt.axhline(y=i, color='lightgray', linewidth=0.5, zorder=0)
        st.pyplot(plt.gcf())
        ...
        for i in range(0, 100, 20):plt.axhline(y=i, color='lightgray', linewidth=0.5, zorder=0)
        plt.gca().set_yticks([])
        plt.tight_layout()
        st.pyplot(plt)
    ...
    with col1:
        ...
        moto_promedio = (sum(suma_victimas_por_anio) / len(suma_victimas_por_anio)).round(2)
